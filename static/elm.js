...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Remote = Elm.Remote.make(_elm),
   $RemoteList = Elm.RemoteList.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StartApp = Elm.StartApp.make(_elm),
   $Task = Elm.Task.make(_elm);
   var handshakeEventsMap = function (list) {
      return $RemoteList.initList(list);
   };
   var connectionEventsMap = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 switch (_v4)
                 {case "join":
                    return $RemoteList.join(_v0._1);
                    case "leave":
                    return $RemoteList.leave(_v0._1);
                    case "noOp":
                    return $RemoteList.noOp;}
                 _U.badCase($moduleName,
   ...
   var connectionEvents = Elm.Native.Port.make(_elm).inboundSignal("connectionEvents",
   "(String, Remote.Model)",
   function (v) {
      return typeof v === "object" && v instanceof Array ? {ctor: "_Tuple2"
                                                           ...
                                                           v[0])
                                                           ,_1: typeof v[1] === "object" && "connection_id" in v[1] ? {_: {}
                                                                                                                      ,connection_id: typeof v[1].connection_id === "string" || typeof v[1].connection_id === "object" && v[1].connection_id instanceof String ? v[1].connection_id : _U.badPort("a string",
                                                                                                                      v[1].connection_id)} : _U.badPort("an object with fields `connection_id`",
                                                           v[1])} : _U.badPort("an array",
      v);
   });
   var connectionEventsSignal = A2($Signal.map,
   connectionEventsMap,
   connectionEvents);
   var handshakeEvents = Elm.Native.Port.make(_elm).inboundSignal("handshakeEvents",
   "List Remote.Model",
   function (v) {
      return typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "connection_id" in v ? {_: {}
                                                                ,connection_id: typeof v.connection_id === "string" || typeof v.connection_id === "object" && v.connection_id instanceof String ? v.connection_id : _U.badPort("a string",
                                                                v.connection_id)} : _U.badPort("an object with fields `connection_id`",
         v);
      })) : _U.badPort("an array",v);
   });
   var handshakeEventsSignal = A2($Signal.map,
   handshakeEventsMap,
   handshakeEvents);
   var app = $StartApp.start({_: {}
                             ,init: $RemoteList.init
                             ,inputs: _L.fromArray([handshakeEventsSignal
...
Elm.Remote = Elm.Remote || {};
Elm.Remote.make = function (_elm) {
   "use strict";
   _elm.Remote = _elm.Remote || {};
   if (_elm.Remote.values)
   return _elm.Remote.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var init = function (x) {
      return Model(x.connection_id);
   };
   _elm.Remote.values = {_op: _op
                        ,init: init
                        ,update: update
                        ,view: view
                        ,Model: Model};
...
Elm.RemoteList = Elm.RemoteList || {};
Elm.RemoteList.make = function (_elm) {
   "use strict";
   _elm.RemoteList = _elm.RemoteList || {};
   if (_elm.RemoteList.values)
   return _elm.RemoteList.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "RemoteList",
   $Basics = Elm.Basics.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var Control = F2(function (a,
   b) {
      return {ctor: "Control"
             ,_0: a
             ,_1: b};
   });
   var viewRemote = F2(function (address,
   remote) {
      return A2($Remote.view,
      A2($Signal.forwardTo,
      address,
      Control(remote.connection_id)),
      remote);
   });
   var view = F2(function (address,
   model) {
   ...
   var Leave = function (a) {
      return {ctor: "Leave",_0: a};
   };
   var leave = function (x) {
      return Leave(x);
   };
   var Join = function (a) {
      ...
      return function () {
         ...
         {case "Control":
            ...
            return noFx(A2($List.map,
              function (z) {
                 return $Remote.init(z);
              },
              action._0));
            case "Join":
            return noFx(A2($List._op["::"],
              action._0,
              model));
            case "Leave":
            return noFx(A2($List.filter,
              function (y) {
                 return !_U.eq(y.connection_id,
                 action._0.connection_id);
              },
              model));
            case "NoOp":
            return noFx(model);}
         _U.badCase($moduleName,
         "between lines 33 and 50");
      }();
   });
   _elm.RemoteList.values = {_op: _op
                            ,noFx: noFx
                            ,init: init
                            ,NoOp: NoOp
                            ,Init: Init
